...
def process_video(video_path , num , videoname , width , height, framerate):
    ...
    while (capture.isOpened()):
        ...
        filename = videoname+'/datapack/'+videoname+'/data/fmm/function'+nottwentyone*'s'+'/tree/frame_'+ str(tempnum) + ".mcfunction"    
        if not readability:
            with open(os.path.join(filename), "w") as noread:
                noread.write("schedule function fmm:tree/frame_0 1t")
            with open(os.path.join(videoname+'/datapack/'+videoname+'/data/fmm/function'+nottwentyone*'s'+"/load.mcfunction"), "w") as loadfunction:
                for h in range(1,width*height+1):
                    if dotsix :
                        texte =str("give @p item_frame[item_name='"+'{"bold":true,"color":"red","text":"Screen {h}"'+"}'"+',entity_data={id:"minecraft:item_frame",Tags:["screen{h}"]}] 1\n')
                    else:
                        texte =str("give @p item_frame{display:{Name:'"+'{"text":"Screen {h}","color":"gray","bold":true}'+"'"+'},EntityTag:{Tags:["screen{h}"]}'+'} 1\n')
                    loadfunction.write(texte.replace("{h}",str(h)))
        ...
        if int(frame_count % frame_skip) == 0:
            with open(os.path.join(filename), "w") as f:
                for i in range(1,width*height+1):
                    ...
                    f.write(cmd + "\n")
                    num += 1
                    vitesse = ((videoframenbr*width*height/frame_skip)-(num-basenum))*((time1-int(time()))/num-basenum)
                    print(num-basenum,"/",videoframenbr*width*height/frame_skip,"≈",round((num-basenum)*frame_skip/(videoframenbr*width*height)*100,2),'%          ',vitesse//60,"minutes et ",vitesse%60," secondes." ,end='\r')
...
if __name__ == "__main__":
    ...
    version = int(input('Do you use ? :\n(1) for 1.20.4 or earlier\n(2) for 1.20.4-1.20.6\n(3) 1.21 or later'))
    dotsix = version>=2
    nottwentyone = version>=3
    framerate = 0
    ...
    os.makedirs(videoname+'/datapack/'+videoname+'/data/fmm/function'+nottwentyone*'s'+'/tree')
    os.makedirs(videoname+'/datapack/'+videoname+'/data/minecraft/tag'+nottwentyone*'s'+'function'+nottwentyone*'s')
    clip = VideoFileClip(video_path)
    audio_clip = clip.audio
    audio_clip.write_audiofile(videoname + "/texture_pack/"+videoname+"/assets/minecraft/sounds/records/wait.ogg")
