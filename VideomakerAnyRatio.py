        ...
        if int(frame_count % frame_skip) == 0:
            with open(os.path.join(filename), "w") as f:
                for i in range(1,width*height+1):
                    ...
                    f.write(cmd + "\n")
                    num += 1
                    print(num-basenum,"/",videoframenbr*width*height/frame_skip,"≈",round((num-basenum)*frame_skip/(videoframenbr*width*height)*100,2),'%          ',end='\r')
...
if __name__ == "__main__":
    ...
    frame_count = 0
    premiere = input('First time ? :')
    install(premiere in ['yes','y','Yes','Y','YES','Oui','oui','O','o'])
